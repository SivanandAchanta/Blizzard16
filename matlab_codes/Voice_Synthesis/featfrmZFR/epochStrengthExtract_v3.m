...
function [zfSig,vnv,vgci,vpc,st,gci]=epochStrengthExtract_v2(wav,fs)
...
% Finding the zero frequency Signal and gci 
	[zfSig, gci, winLength]=epochExtract_v2(wav,fs);
		
	winLength %= 4;

%	wav1=resample(wav,8000,16000); %vnvseg works best for 8k.
	[vnvsig] = vnvseg(wav,zfSig,fs,winLength);
