...
function [lpr,zfSig,vnv,vgci,vpc,st]=epochStrengthExtract_lpr(wav,fs)
...
% Finding the zero frequency Signal and gci 
	[lpr,zfSig, zfSig_orig, gci, winLength]=epochExtract_lpr(wav,fs);
		
	winLength

%	wav1=resample(wav,8000,16000); %vnvseg works best for 8k.
	[vnvsig] = vnvseg(wav,zfSig_orig,fs,winLength);
...
%%%%%%%%%%%%%%%%%%%%% Getting voiced strengths
	st=zfSig_orig(vgci)-zfSig_orig(vgci-1);
